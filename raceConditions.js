...
function runSimulation() {
        ...
        setTimeout(() => {
          ...
          if (entradas.length !== 0) {
            if (cantidadEntradas <= entradas.length) {
              ...
              permitirCompraEntradasRestantes(
                usuario,
                ticket,
                cantidadEntradas,
                resolve,
   ...
   */
  function permitirCompraEntradasRestantes(
    usuario,
    ticket,
    cantidadEntradas,
    resolve,
    ...
    do {
      try {
        console.log(
          `Lo siento ${usuario}, no hay suficientes entradas. Si desea comprar entradas igualmente, ingrese cuántas entradas desea comprar. Si no desea comprar ninguna entrada, ingrese 0.`
        );
        comprarEntradasRestantes = Number(
          prompt(`(${entradas.length} entradas restantes): `)
        );
        if (
          comprarEntradasRestantes % 1 !== 0 ||
          comprarEntradasRestantes < 0
        ) {
      ...
      typeof comprarEntradasRestantes !== "number" ||
      comprarEntradasRestantes % 1 !== 0 ||
      comprarEntradasRestantes > entradas.length
    );
...
const Stock = {
  ...
  disminuir(cantidad) {
    ...
    if (cantidad > 6)
      throw new Error("No se pueden comprar más de 6 entradas a la vez");
